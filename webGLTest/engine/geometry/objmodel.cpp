...
{
	if( !load() ) {
		std::cerr << "ObjModel: Failed to load: " << objFile << std::endl;
	}
}
...
bool ObjModel::load() {
	...
	*/
	...
	// Translate the separate obj indexes to vertex layout
...
void ObjModel::objTexCoord(std::string line) {
...
void ObjModel::objVertNormal(std::string line) {
...
void ObjModel::objFace(std::string line) {
  ...
  // Split on space to extract face vertices
	...
	{
		auto part = parts[i];
		// Split on slash to extract per-vertex info
		std::sregex_token_iterator it2{part.begin(), part.end(), regexSlash, -1};
		std::vector<std::string> parts2{it2, {}};

		glm::vec<3,GLuint> face;
		face[0] = static_cast<GLuint>(std::stoul(parts2[0])); // Vert Index
		// If there's 3 parts it means all parameters were specified, else just the coord/normal
		if( parts2.size() == 3 ) {
			face[1] = static_cast<GLuint>(std::stoul(parts2[1])); // TexCoord Index
			face[2] = static_cast<GLuint>(std::stoul(parts2[2])); // Norm Index
		} else {
			face[2] = static_cast<GLuint>(std::stoul(parts2[1])); // Norm Index
		}
...
void ObjModel::objVert(std::string line) {
